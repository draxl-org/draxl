...
- semantic conflicts are not implemented yet
...
The Git reproduction test proves a motivating case on the text side.
...
- add a Draxl fixture for the verified Git merge example
- add semantic-merge tests that use the fixture directly
- keep the fixture small enough to read quickly
...
3. Add the first semantic rule:
   `binding_rename_vs_meaning_change`.
4. Add Draxl fixtures for the verified Git example.
5. Add more semantic rules only after the first one produces useful output.

## Acceptance Bar For The First Semantic Rule

Before calling the first semantic rule solid, it should:

- detect the motivating rename-vs-meaning example
- explain why the conflict is semantic rather than hard
- point to the relevant binding and both patch-side edits
