...
#[test]
fn mcp_setup_print_emits_managed_codex_config() {
    let root = write_temp_dir("mcp_setup_print");
    let canonical_root = root.canonicalize().expect("temp dir should canonicalize");
    ...
    assert!(output.starts_with("# Generated by `draxl mcp setup --client codex`."));
    assert!(output.contains("sandbox_mode = \"read-only\""));
    assert!(output.contains("approval_policy = \"on-request\""));
    assert!(output.contains("allow_login_shell = false"));
    assert!(output.contains("Use the `draxl_*` MCP tools for edits to `.rs.dx` files."));
    assert!(output.contains("[mcp_servers.draxl]"));
    assert!(output.contains("args = [\"mcp\", \"serve\", \"--root\""));
    assert!(output.contains(&canonical_root.display().to_string()));
    assert!(output.contains("[mcp_servers.draxl.tools.draxl_inspect_file]"));
    assert!(output.contains("[mcp_servers.draxl.tools.draxl_get_node]"));
    assert!(output.contains("[mcp_servers.draxl.tools.draxl_check_conflicts]"));
}

#[test]
fn mcp_setup_writes_config_and_updates_git_exclude() {
    let root = write_temp_dir("mcp_setup_write");
    git_init(&root);
    ...
    let exclude = fs::read_to_string(&exclude_path).expect("exclude should exist");

    assert!(output.contains(&format!("wrote {}", config_path.display())));
    assert!(config.starts_with("# Generated by `draxl mcp setup --client codex`."));
    assert!(exclude.lines().any(|line| line.trim() == ".codex/"));
}

#[test]
fn mcp_setup_refuses_to_overwrite_unmanaged_config_without_force() {
    let root = write_temp_dir("mcp_setup_refuse");
    fs::create_dir_all(root.join(".codex")).expect("codex dir should exist");
    fs::write(
        root.join(".codex/config.toml"),
        "approval_policy = \"never\"\n",
    )
    .expect("unmanaged config should write");
    ...
    assert!(output.contains("refusing to overwrite unmanaged Codex config"));
    assert!(output.contains("--force"));
}

#[test]
fn mcp_setup_force_overwrites_unmanaged_config() {
    let root = write_temp_dir("mcp_setup_force");
    fs::create_dir_all(root.join(".codex")).expect("codex dir should exist");
    fs::write(
        root.join(".codex/config.toml"),
        "approval_policy = \"never\"\n",
    ...
    let config = fs::read_to_string(root.join(".codex/config.toml")).expect("config should exist");
    assert!(config.starts_with("# Generated by `draxl mcp setup --client codex`."));
    assert!(config.contains("[mcp_servers.draxl]"));
}
